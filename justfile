...
update-pdf:
    typst compile ./template/main.typ {{ vision_pdf }}

watch-pdf:
    typst watch ./template/main.typ {{ vision_pdf }}

update-png:
    typst compile --pages=1 ./template/main.typ {{ thumbnail_png }}

watch-png:
    typst watch --pages=1 ./template/main.typ {{ thumbnail_png }}

watch:
    watchexec -w ./lib -w ./template -w ./typst.toml -w {{ thumbnail_png }} just deploy
...
deploy:
    ...
    @if cp {{ thumbnail_png }} {{ local-lib }} ; then echo "{{ BG_GREEN }}Success copying {{ thumbnail_png }} {{ NORMAL }}" ; else echo "{{ BG_RED }}ERROR copying {{ thumbnail_png }} {{ NORMAL }}"; fi
